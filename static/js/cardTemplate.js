function escapeHtml(value) {
  return String(value ?? '').replace(/[&<>"']/g, (ch) => ({
    '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;',
  }[ch]));
}

const HEALTH_LABEL = {
  healthy: 'Healthy',
  progressing: 'Progressing',
  degraded: 'Degraded',
  suspended: 'Suspended',
  unknown: 'Unknown',
};

// Kinds rendered in the single-wide-column list layout (see graphView.js's
// SINGLE_COLUMN_LIST_TYPES) get a wider card so their full name is visible.
const WIDE_CARD_KINDS = ['CustomResourceDefinition'];

function renderCard(node) {
  const icon = ICONS[node.icon] || ICONS.default;
  const badges = (node.badges || [])
    .map((b) => `<span class="krw-badge krw-badge-${escapeHtml(b.variant)}">${escapeHtml(b.text)}</span>`)
    .join('');
  const healthLabel = HEALTH_LABEL[node.health] || node.health;
  // Cluster-scoped kinds (Node, PersistentVolume, CustomResourceDefinition)
  // have no namespace -- omit the line entirely rather than showing it empty.
  const namespaceLine = node.namespace
    ? `<div class="krw-card-namespace" title="${escapeHtml(node.namespace)}">${escapeHtml(node.namespace)}</div>`
    : '';

  return `
    <div class="krw-card ${node.is_root ? 'is-root' : ''} ${node.is_selected ? 'is-selected' : ''} ${WIDE_CARD_KINDS.includes(node.kind) ? 'krw-card--wide' : ''}">
      <div class="krw-card-icon">${icon}</div>
      <div class="krw-card-body">
        <div class="krw-card-title" title="${escapeHtml(node.name)}">${escapeHtml(node.name)}</div>
        <div class="krw-card-kind">${escapeHtml(node.kind)}</div>
        ${namespaceLine}
        <div class="krw-card-badges">
          <span class="krw-dot krw-dot-${escapeHtml(node.health)}" title="${escapeHtml(healthLabel)}"></span>
          ${badges}
        </div>
      </div>
    </div>
  `;
}
