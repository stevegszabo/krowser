...
function renderCard(node) {
  ...
  const kindLabel = node.is_static ? `${node.kind} (static)` : node.kind;
  // Cluster-scoped kinds (Node, PersistentVolume, and cluster-scoped custom
  // resources) have no namespace -- omit the line entirely rather than
  // showing it empty.
  ...
  return `
    <div class="krw-card ${node.is_root ? 'is-root' : ''} ${node.is_selected ? 'is-selected' : ''}">
      ...
      <div class="krw-card-body">
        ...
        <div class="krw-card-kind">${escapeHtml(kindLabel)}</div>
        ${namespaceLine}
