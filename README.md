...
- PersistentVolumeClaim/PersistentVolume tiles show live volume usage (e.g. `51Gi / 434Gi (11%)`) on the PersistentVolumes and PersistentVolumeClaims views, read from each node's kubelet Summary API since metrics-server never exposed volume stats; a volume ≥90% full is flagged as a degraded "Nearly full" health signal — a common precursor to "pod stuck, disk full" incidents that's otherwise invisible until something actually breaks
- Pod tiles on the Pods view also show live ephemeral-storage usage (e.g. `Disk: 204Mi`) — the writable container layer, logs, and emptyDir volumes combined, exactly what the kubelet sums to decide whether to evict a pod under DiskPressure; when a container sets an `ephemeral-storage` limit, this shows as a percentage instead and flags ≥90% as a degraded "Disk nearly full" signal
